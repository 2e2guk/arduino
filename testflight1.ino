...
// 알림 임계값
float threshold = 0.5; // 알림을 위한 임계값 (필요에 따라 조정)

// 보호자 정보
String guardianName = "이강욱";
String guardianPhoneNumber = "010-6284-2718";
...
// 알림 횟수를 저장할 변수
int alertCount = 0; // 119 및 보호자 알림 횟수 제한
...
void loop() {
  ...
  if (alertval >= threshold) {
    if (!conditionMet) {
      ...
      if (elapsedTime >= 1000 && alertCount < 2) { // 1초 이상 지속 + 알림 횟수 제한 -> 실제 코드에서는 위험 수준이 1분 이산 지속이 신고. 
        // elaspsedTime = 60000
        alert119();
...
void initializeTemperatureSensor() {
  ...
  if (mlx.begin()) {
    ...
    Serial.println("--------------------체온, 심박수 데이터 수집을 시작합니다.-------------------");
  } else {
